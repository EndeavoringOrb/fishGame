...
// Return a random RGB color with alpha=255
sf::Color randColor(uint32_t &randSeed)
{
    return {(sf::Uint8)randInt(randSeed, 256), (sf::Uint8)randInt(randSeed, 256), (sf::Uint8)randInt(randSeed, 256)};
}

glm::vec3 randVec3(uint32_t &randSeed)
{
    return {randFloat(randSeed), randFloat(randSeed), randFloat(randSeed)};
}
